...
- The local Sub2API provider uses the native Gemini /antigravity/v1beta generateContent route. Keep its credentials and base URL in runtime configuration, identify model choices by provider plus model, and do not reuse official Gemini Interactions or explicit-cache state for Sub2API sessions.
