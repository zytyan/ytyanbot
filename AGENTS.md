...
- Checksummed migration schema text is immutable. V3 uses the frozen `sql/migrate/schema_ai_v2_v3.sql`; canonical schema changes belong in later migrations and must never change an applied version's checksum.
...
- Main schema V10 retains Telegram AI photo-album metadata for seven days. Capture media groups only in AI-enabled chats, mark mixed groups unsupported, store Telegram file identifiers rather than downloading media eagerly, and cascade expiry cleanup to album items.
- Main schema V11 stores each Telegram user's global AI reaction preference in `ai_user_settings`, separate from the lightweight `users` name dimension. Missing settings default to reactions enabled.
...
- The local Sub2API provider uses the native Gemini /antigravity/v1beta generateContent route. Keep its credentials and base URL in runtime configuration, identify model choices by provider plus model, map the generic Gemini 3.7 Flash selection to the tiered Antigravity model, and do not reuse official Gemini Interactions or explicit-cache state for Sub2API sessions.
- Deliver final AI text with `sendRichMessage` using Telegram Rich Markdown, preserving reply parameters, the usage keyboard, and the returned Telegram message identity used by Run delivery. Keep ordinary commands and error messages on `sendMessage`; do not route RichMessage output through `helpers/mdnormalizer` unless a new fallback requirement explicitly calls for it. The default AI prompt may use tables, formulas, details, and other Rich Markdown blocks.
